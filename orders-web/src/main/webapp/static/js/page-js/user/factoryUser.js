layui.use(['table','laydate'], function(){
  ...
  var tableIns = table.render({
    elem: '#userTableId'
    ,url:'user/selecFactoryUserAndUnit'
    ,headers:{
    ...
    ,cols: [[
      ...
      ,{field:'cz', title:'操作',fixed: 'right',width:150,templet: '#lineToolbar'}
    ]]
    ,page: true
  });
  ...
  var unitList = function(){
	  $.ajax({
		  url:'unit/getFactoryUnitList'
		  ,type:'post'
		  ,dataType:'json'
		  ,success:function(result){
			  if(10000 == result.code){
				  $("#unitList").empty();
				  $("#unitList").append('<option value="">请选择工厂名称</option>');
